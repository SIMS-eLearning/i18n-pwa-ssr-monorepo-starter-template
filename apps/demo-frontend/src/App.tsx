...
const App: React.FC<Props> = ({ assetMap }) => {
  ...
  useEffect(() => {
    if (assetMap && assetMap.initialLanguage) {
      i18n.changeLanguage(assetMap.initialLanguage);
    }
  }, [assetMap]);
  //to prevent inifinte renders
  ...
  const appBody = () => {//can be used at DEV time and PROD time
    ...
    //console.log(`assetMap in App = ${JSON.stringify(assetMap)}`)
    return (
